...
esperar_n_nodos(Servidores, NumInstancia, N_Deseados) ->
    ...
    timer:sleep(15),  %% en milisegundos
...
instancias_fuera_orden() -> 
    ...
    if MaxNuInst =/= 7 -> ?debugFmt("max(~p) erróneo~n",['n1@127.0.0.1']);
       true -> ok
    end,
...
olvidando() ->
   ...
   % Crear 6 nodos Paxos
    ...
    %%%%% min() Inicial    ?debugFmt("Comprobar min() inicial ---------~n",[]),
...
particionar(Lista_particiones) ->
    lists:foreach(fun(P) ->
                    lists:foreach(fun(X) -> paxos:limitar_acceso(X, P) end, P)
                  end,
                  Lista_particiones),
